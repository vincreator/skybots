...
class LiveClient(Session):
            ...
            if data.get("user"):
                ...
                if data.get("live"):
                    ...
                    if not live.get("secret"):
                        result["stream_url"] = live.get("pullUrlList", [])[0]
                else:
                    result |= {"online": False, "thumbnail": users.get("head")}
    ...
    def getThai69LiveUser(self, room_id):
        ...
        r = self.request(
            "POST",
            f"{self.host_url}/intem-live/index/user_video_async",
            content=json.dumps(data),
            headers=self.headers,
        )
        if r.status_code == 200:
            ...
            if data.get("code") == 200:
                ...
                result = {
                    ...
                    "secret": bool(data.get("is_live_pay")),
                ...
                try:
                    ...
                    if r.status_code == 200:
                        with open(f"{room_id}.jpg", "wb") as fp:
                            fp.write(r.content)
                        result["thumbnail"] = f"{room_id}.jpg"
                    else:
    ...
    def getHoneyRecommended(self, page = 1):
        ...
        r = self.request(
            'GET',
            f"{self.host_url}/App/Live/RecommendList",
            params=params,
            headers=self.headers,
        )
            ...
            result = [
                {
                    ...
                    "secret": bool(float(data.get('price'))),
                    "thumbnail": f"https://stdn-android.bugly.qq.com/{data.get('cover')}"
                    if data.get('cover')
                    else None,
                    "live_id": data.get("cover")
                    .replace("live/cover/20211209/", "")
                    .split("_")[1]
                    .replace(".png", "")
                    if data.get("cover")
                    else None,
    ...
    def getHoneyLiveUser(self, id):
        ...
        return (
            json.dumps(result[0])
            if result
            else json.dumps({"code": 404, "result": "Unknown maybe host offline"})
        )
